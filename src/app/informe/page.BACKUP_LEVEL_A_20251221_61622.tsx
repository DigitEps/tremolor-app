...
export default function InformePage() {
  ...
  // ---------- gating (Nivell A vs PRO) ----------
  // IMPORTANT:
  // - Aquesta pàgina està pensada per funcionar 100% en estàtic (cPanel) a Nivell A.
  // - El backend (API / email / PDF PRO) es reactivarà més endavant. No el perdem: el mantenim aparcat al repo.
  const [error, setError] = useState<string | null>(null);
  const [hasPerk, setHasPerk] = useState<boolean>(false); // es posa a true quan l'usuari entra el codi del llibre (/regal)
  const canPro = hasPerk;
  ...
  const copyLevelA = async ()=>{
    // Copia el Nivell A al porta-retalls (si falla, baixa un TXT)
    try { 
      await navigator.clipboard.writeText(previewMd);
      alert("Informe copiat. Ara el pots enganxar on vulguis.");
    } catch {
      download("tremolor-informe.txt", previewMd, "text/plain;charset=utf-8");
    }
  };

  const downloadLevelATxt = ()=>download("tremolor-informe.txt", previewMd, "text/plain;charset=utf-8");

  const downloadProTxt = ()=>{
    if (!canPro) { setError("Per descarregar el PRO, entra el codi del llibre (apartat /regal)."); return; }
    download("tremolor-pla-pro.txt", fullPlanMd, "text/plain;charset=utf-8");
  };

  const downloadProMd = ()=>{
    if (!canPro) { setError("Per descarregar el PRO, entra el codi del llibre (apartat /regal)."); return; }
    download("tremolor-pla-pro.md", fullPlanMd, "text/markdown;charset=utf-8");
  };
  ...
  return (
    <main className="min-h-screen px-6 py-12 bg-gradient-to-br from-gray-900 via-black to-purple-900 text-white">
      <div className="max-w-6xl mx-auto space-y-12">
        ...
        {/* Nivell A / PRO (sense email ni PDF a la versió estàtica) */}
        <section className="bg-white/5 border border-white/10 rounded p-4 space-y-3">
          <div className="text-sm opacity-90">
            <strong>Nivell A (gratis):</strong> informe immediat i aplicable. Sense correus, sense PDFs, sense fum.
          </div>

          <div className="flex flex-col sm:flex-row gap-2">
            <button
              onClick={copyLevelA}
              className="px-4 py-2 rounded-lg bg-white text-black font-medium hover:bg-white/90 text-sm transition-colors"
            >
              📋 Copiar informe (Nivell A)
            </button>

            <button
              onClick={downloadLevelATxt}
              className="px-4 py-2 rounded-lg bg-white/10 hover:bg-white/15 text-white border border-white/10 text-sm transition-colors"
              title="Descarrega un TXT (s'obre a qualsevol mòbil o PC)"
            >
              ⬇️ Descarregar TXT
            </button>
          </div>
          ...
          <div className="text-xs opacity-80">
            <span className="font-semibold">PRO (codi del llibre):</span>{" "}
            {canPro ? (
              <span className="text-green-400">✅ activat</span>
            ) : (
              <span className="text-yellow-300">bloquejat</span>
            )}
            {" "}
            — <a href="/regal" className="underline underline-offset-4 hover:opacity-80">entra el codi</a>
          </div>

          {canPro && (
            <div className="flex flex-col sm:flex-row gap-2 pt-2 border-t border-white/10">
              <button
                onClick={downloadProTxt}
                className="px-4 py-2 rounded-lg bg-white/10 hover:bg-white/15 text-white border border-white/10 text-sm transition-colors"
                title="Pla PRO en TXT (fàcil d'obrir)"
              >
                ⬇️ Pla PRO (TXT)
              </button>

              <button
                onClick={downloadProMd}
                className="px-4 py-2 rounded-lg bg-white/10 hover:bg-white/15 text-white border border-white/10 text-sm transition-colors"
                title="Pla PRO en Markdown (per guardar o importar)"
              >
                📝 Pla PRO (MD)
              </button>
              ...
              <button
                disabled
                className="px-4 py-2 rounded-lg bg-white/5 opacity-50 cursor-not-allowed border border-white/10 text-sm"
                title="El PDF PRO necessita backend. El reactivarem quan toqui."
              >
                📄 PDF PRO (aviat)
              </button>
            </div>
